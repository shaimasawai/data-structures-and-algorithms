...
const toLastNames = (people) => {
  ...
  let arrNew = [];
  people.map((ele) => {
    arrNew.push(ele.firstName + " " + ele.lastName);
  });
...
const addValues = (arr) => {
  // Solution code here...
  let sumation = arr.reduce((acc, val) => {
    return acc + val;
  }, 0);
  return sumation;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let iterateArr = arr.reduce((accNum) => {
    accNum = accNum + 1;
    return accNum;
  }, 0);
  return iterateArr;
...
const returnNames = (arr) => {
  // Solution code here...
  let newArryret = arr.reduce((acc, val, idx) => {
    acc[idx] = val.name;
    return acc;
  }, []);
  return newArryret;
...
const reversedString = (str) => {
  // Solution code here...
  let reversArr = str.split("").reduce((acc, item) => {
    acc = item + acc;

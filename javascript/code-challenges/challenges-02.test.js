...
const raisedToTheThird = (arr) => {
  ...
  let arraynumber = [];
  arr.forEach(element =>{
    arraynumber.push(Math.pow(element,3));
  });
  return arraynumber;
};
...
const addOne = (arr) => {
  // Solution code here...
  let arraynew = arr.map( element=>{return element + 1; });
  return arraynew;
...
const addQuestion = (arr) => {
  // Solution code here...
  let arrayquest= arr.map( element=>{return element + '?'; });
  return arrayquest;
};

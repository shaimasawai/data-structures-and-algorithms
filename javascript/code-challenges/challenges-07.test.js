...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  const height = starWarsArr.sort((x, y) => {
...
const listFoods = (recipe) => {
  ...
  for (let x = 0; x < recipe.ingredients.length; x++) {
    var counter = recipe.ingredients[x].split(" ").length - 1;
    if (counter === 2) {
      result.push(
        recipe.ingredients[x].slice(recipe.ingredients[x].lastIndexOf(" ") + 1)
      );
    } else {
      var indexnew = recipe.ingredients[x].indexOf(
        " ",
        recipe.ingredients[x].indexOf(" ") + 1
      );

      if (indexnew >= 0)
        var secound = recipe.ingredients[x].substr(indexnew + 1);
